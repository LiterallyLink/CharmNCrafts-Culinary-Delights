...
/**
 * Fried Dumpling block that requires a bowl to eat.
 * Gives Bowl of Fried Dumpling when right-clicked with a bowl.
 * On the final serving, drops only a bowl (no food) when destroyed.
 */
public class FriedDumplingBlock extends Block {
    public static final DirectionProperty FACING = Properties.HORIZONTAL_FACING;
    public static final IntProperty SERVINGS = IntProperty.of("servings", 0, 4);
    
    public FriedDumplingBlock(Settings settings) {
        super(settings);
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        ...
        if (newServings < 0) {
            ...
            // Not the last serving - give Bowl of Fried Dumpling
            ItemStack bowlOfFriedDumpling = new ItemStack(ModItems.BOWL_OF_FRIED_DUMPLING);
            if (!player.getInventory().insertStack(bowlOfFriedDumpling)) {
                player.dropItem(bowlOfFriedDumpling, false);
            }
